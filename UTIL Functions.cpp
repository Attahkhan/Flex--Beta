...
Vector GetHitBoxPosV2(IClientEntity* pEntity, int Hitbox)
{
	...
	if (pEntity->SetupBones(matrix, MAXSTUDIOBONES, BONE_USED_BY_HITBOX, GetTickCount64()))
	{
		...
		if (hitbox)
		{
			...
			if (Menu::Window.RageBotTab.TargetMultipoint.GetState()) // Ghetto Ass Multipoint
			{
				int iCount = 7;
				...
				for (int i = 0; i <= iCount; i++)
				{
					...
					{
					case 1:
						vCenter = (Point[7] + Point[1]) * Menu::Window.RageBotTab.TargetPointscale.GetValue();
						break;
					case 2:
						vCenter = (Point[3] + Point[4]) * Menu::Window.RageBotTab.TargetPointscale.GetValue();
						break;
					case 3:
						vCenter = (Point[4] + Point[0]) * Menu::Window.RageBotTab.TargetPointscale.GetValue();
						break;
					case 4:
						vCenter = (Point[2] + Point[7]) * Menu::Window.RageBotTab.TargetPointscale.GetValue();
						break;
					case 5:
						vCenter = (Point[6] + Point[7]) * Menu::Window.RageBotTab.TargetPointscale.GetValue();
						break;
					case 6:
						vCenter = (Point[5] + Point[3]) * Menu::Window.RageBotTab.TargetPointscale.GetValue();
						break;
					case 7:
						vCenter = (Point[1] + Point[2]) * Menu::Window.RageBotTab.TargetPointscale.GetValue();
						break;
					}
					return vCenter;
				}
...
Vector GetHitboxPosition(IClientEntity* pEntity, int Hitbox)
{
	...
	if (pEntity->SetupBones(matrix, 128, 256, Interfaces::Engine->GetLastTimeStamp()))
	{
		studiohdr_t* hdr = Interfaces::ModelInfo->GetStudiomodel(pEntity->GetModel());
		mstudiohitboxset_t* set = hdr->GetHitboxSet(0);
		...
		if (hitbox)
		{
			...
			{
				for (auto i = 0; i <= Menu::Window.RageBotTab.MultipointIndexes.GetValue() + 1; i++) // multipoint type is the indexes go from 0,25
				{
					if (CanHit(vCenter + MultipointVectors[i], &Damage))
